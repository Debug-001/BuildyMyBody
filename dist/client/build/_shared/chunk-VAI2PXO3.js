...
function we(e) {
  ...
  for (let a of Object.keys(e))
    switch (a) {
      case "title": {
        let r = v(L.title, e.title), i = yt(e?.titleTemplate, r);
        if (!i)
          break;
        t.push(C("title", { title: i }), C("meta", { property: "og:title", content: i }), C("meta", { name: "twitter:title", content: i }));
        break;
      }
      case "description": {
      ...
      case "media": {
        let r, i = X(e.media);
        for (let n of i)
          if (typeof n == "string" && t.push(C("meta", { name: "og:image", content: n })), n && typeof n == "object") {
            let o = n.type || "image", s = n ? { url: n?.url, secure_url: n?.url, type: ft(n.url), width: n?.width, height: n?.height, alt: n?.altText } : {};
            for (let u of Object.keys(s))
              s[u] && (r = s[u], t.push(C("meta", { property: `og:${o}:${u}`, content: r }, s.url)));
          }
        break;
      }
      case "jsonLd": {
        let r = X(e.jsonLd), i = 0;
        for (let n of r) {
          if (typeof n != "object")
            continue;
          let o = C("script", { type: "application/ld+json", children: JSON.stringify(n) }, `json-ld-${n?.["@type"] || n?.name || i++}`);
          t.push(o);
        }
        break;
      }
      case "alternates": {
        ...
        for (let i of r) {
          if (!i)
            continue;
          let { language: n, url: o, default: s } = i, u = n ? `${n}${s ? "-default" : ""}` : void 0;
          t.push(C("link", { rel: "alternate", hrefLang: u, href: o }));
        }
        break;
      }
      case "robots": {
  ...
  return t.flat().sort((a, r) => a.key.localeCompare(r.key));
}
function C(e, t, a) {
  let r = { tag: e, props: {}, key: "" };
  return e === "title" ? (r.children = t.title, r.key = z(r), r) : e === "script" ? (r.children = typeof t.children == "string" ? t.children : "", r.key = z(r, a), delete t.children, r.props = t, r) : (r.props = t, Object.keys(r.props).forEach((i) => !r.props[i] && delete r.props[i]), r.key = z(r, a), r);
}
function z(e, t) {
  let { tag: a, props: r } = e;
  if (a === "title")
    return "0-title";
  if (a === "meta") {
    let i = r.content === t && typeof r.property == "string" && !r.property.endsWith("secure_url") && "0";
    return [a, ...[t, i], r.property || r.name].filter((o) => o).join("-");
  }
  return a === "link" ? [a, r.rel, r.hrefLang || r.media].filter((n) => n).join("-").replace(/\s+/g, "-") : a === "script" ? `${a}-${t}` : `${a}-${r.type}`;
}
function yt(e, t) {
  if (!!t)
...
function v(e, t) {
  try {
    ...
    return console.warn(a.message), t;
  }
}
var It = (0, import_react6.lazy)(() => import("/build/_shared/log-seo-tags-N7J66FN6-VDGRBTLH.js"));
function Rt({ debug: e }) {
  let t = useMatches(), a = useLocation(), r = (0, import_react6.useMemo)(() => t.flatMap((o) => {
    let { handle: s, ...u } = o, c = { ...u, ...a }, p = s?.seo, d = u?.data?.seo;
    return !p && !d ? [] : p ? N(s.seo, c) : [d];
  }).reduce((o, s) => {
    Object.keys(s).forEach((c) => !s[c] && delete s[c]);
    let { jsonLd: u } = s;
    return u ? o?.jsonLd ? Array.isArray(u) ? { ...o, ...s, jsonLd: [...o.jsonLd, ...u] } : { ...o, ...s, jsonLd: [...o.jsonLd, u] } : { ...o, ...s, jsonLd: [u] } : { ...o, ...s };
  }, {}), [t, a]), { html: i, loggerMarkup: n } = (0, import_react6.useMemo)(() => {
    let o = we(r), s = o.map((c) => c.tag === "script" ? (0, import_react6.createElement)(c.tag, { ...c.props, key: c.key, dangerouslySetInnerHTML: { __html: c.children } }) : (0, import_react6.createElement)(c.tag, { ...c.props, key: c.key }, c.children)), u = (0, import_react6.createElement)(import_react6.Suspense, { fallback: null }, (0, import_react6.createElement)(It, { headTags: o }));
    return { html: s, loggerMarkup: u };
  }, [r]);
  return (0, import_react6.createElement)(import_react6.Fragment, null, i, e && n);
}
function N(e, ...t) {
  if (e instanceof Function)
    return N(e(...t), ...t);
  let a = {};
  return Array.isArray(e) ? (a = e.reduce((r, i) => [...r, N(i)], []), a) : e instanceof Object ? (Object.entries(e).forEach(([i, n]) => {
    a[i] = N(n, ...t);
  }), a) : e;
}
function At({ connection: e, children: t = () => (console.warn("<Pagination> requires children to work properly"), null) }) {
  let r = useNavigation().state === "loading", { endCursor: i, hasNextPage: n, hasPreviousPage: o, nextPageUrl: s, nodes: u, previousPageUrl: c, startCursor: p } = Ot(e), d = (0, import_react6.useMemo)(() => ({ pageInfo: { endCursor: i, hasPreviousPage: o, startCursor: p }, nodes: u }), [i, o, p, u]), l = (0, import_react6.useMemo)(() => function(m) {
    return n ? (0, import_react6.createElement)(Link, { preventScrollReset: true, ...m, to: s, state: d, replace: true }) : null;
  }, [n, s]), h = (0, import_react6.useMemo)(() => function(m) {
    return o ? (0, import_react6.createElement)(Link, { preventScrollReset: true, ...m, to: c, state: d, replace: true }) : null;
  }, [o, c]);
...
function Ot(e) {
  let { state: t, search: a } = useLocation(), n = new URLSearchParams(a).get("direction") === "previous", o = (0, import_react6.useMemo)(() => !t || !t?.nodes ? flattenConnection(e) : n ? [...flattenConnection(e), ...t.nodes] : [...t.nodes, ...flattenConnection(e)], [t, e]), s = (0, import_react6.useMemo)(() => {
  ...
  }, [n, t, e.pageInfo.hasNextPage, e.pageInfo.hasPreviousPage, e.pageInfo.startCursor, e.pageInfo.endCursor]), u = (0, import_react6.useMemo)(() => {
    let p = new URLSearchParams(a);
    return p.set("direction", "previous"), s.startCursor && p.set("cursor", s.startCursor), `?${p.toString()}`;
  }, [a, s.startCursor]), c = (0, import_react6.useMemo)(() => {
    let p = new URLSearchParams(a);
...
function F({ children: e, action: t, inputs: a, route: r }) {
  let i = useFetcher();
  return (0, import_jsx_runtime9.jsxs)(i.Form, { action: r || "", method: "post", children: [(t || a) && (0, import_jsx_runtime9.jsx)("input", { type: "hidden", name: Ne, value: JSON.stringify({ action: t, inputs: a }) }), typeof e == "function" ? e(i) : e] });
}
F.INPUT_NAME = Ne;
...
*/
//# sourceMappingURL=/build/_shared/chunk-VAI2PXO3.js.map
